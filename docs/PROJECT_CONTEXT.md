...
- 添加食物页：语音录入、拍日期识别、手动录入，全部进入确认表单后保存；拍日期优先走服务端大模型，失败回退本地 OCR。
- 添加位置页：用户命名位置、添加标签、上传照片，生成简笔画风格封面。
- 操作记录页和家庭成员页；家庭页提供试用邀请码 `WOJIA`，`/join` 支持输入邀请码和称呼加入 demo 家庭。
- MVP E2E 测试：从提醒中心到位置、记录、家庭页，并覆盖试用邀请码加入家庭流程。
...
- `npm run test`: 13 个测试文件，31 个测试通过。
- `npm run build`: 通过。
...
- 本地 OCR 是兜底方案，识别结果必须人工确认。OpenAI API 需要有效额度，国内网络环境通常还要配置 `OPENAI_PROXY_URL`。
...
3. 优化添加食物流：添加成功后清空表单、跳转到对应柜子或快到期页。
