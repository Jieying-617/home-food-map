...
test("demo family can move through the trial MVP screens", async ({ page }) => {
  ...
  await expect(page.getByText(/AI 卡通封面预览|原图预览/)).toBeVisible();
  await expect(page.getByRole("img", { name: "位置封面预览" })).toBeVisible();
  ...
  await expect(page.getByText(/AI 卡通封面预览已生成|AI 生成不可用|正在生成真正的卡通图/)).toBeVisible();
  await expect(page.getByRole("img", { name: "新的位置封面预览" })).toBeVisible();
  await page.getByRole("button", { name: "保存新图片" }).click();
  await expect(page.getByText("位置图片已更换。")).toBeVisible();
  await page.getByRole("link", { name: "位置" }).click();
